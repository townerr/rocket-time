...
export const managerRouter = createTRPCRouter({
  ...
  getAllEmployeeTimesheets: protectedProcedure
    .query(async ({ ctx }) => {
  ...
  getEmployeeHoursStats: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const userIds = allUsers.map(user => user?.id).filter(Boolean) as string[];
      
      console.log(`Analyzing timesheets for ${userIds.length} users (manager + employees)`);
      ...
      console.log(`Found ${timesheets.length} timesheets in the selected date range`);
      ...
      console.log(`Returning data for ${result.length} users with hours:`, result);
      return result;
    }),
  ...
  getProjectDistribution: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      console.log(`Analyzing projects for ${userIds.length} users (manager + employees)`);
      ...
      console.log(`Found ${timesheets.length} timesheets with ${timesheets.reduce((count, ts) => count + ts.entries.length, 0)} entries`);
      ...
      timesheets.forEach(timesheet => {
        timesheet.entries.forEach(entry => {
          ...
          if (projectMap.has(projectCode)) {
            projectMap.set(projectCode, projectMap.get(projectCode) + entry.hours);
          } else {
  ...
  getTimesheetStatusStats: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      console.log(`Analyzing timesheet statuses for ${userIds.length} users (manager + employees)`);
  ...
  updateEmployeeStatus: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!employee) {
        throw new Error("Employee not found or you don't have permission to manage this employee");
      }
  ...
  // WorkType Management
  getAllWorkTypes: protectedProcedure
    .query(async ({ ctx }) => {
